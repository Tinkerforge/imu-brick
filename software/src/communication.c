...
extern uint8_t imu_range_acc;
extern uint8_t imu_range_mag;
...
void set_acceleration_range(const ComType com, const SetAccelerationRange *data) {
	...
	imu_range_acc = data->range;
	imu_update_range_acc();
...
void set_magnetometer_range(const ComType com, const SetMagnetometerRange *data) {
	if(data->range == 0 || data->range > 7) {
		com_return_error(data, sizeof(MessageHeader), MESSAGE_ERROR_CODE_INVALID_PARAMETER, com);
		return;
	}

	imu_range_mag = data->range;
	imu_update_range_mag();
